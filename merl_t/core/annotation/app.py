...
from merl_t.config.config_manager import get_config_manager
from .db_manager import AnnotationDBManager
from merl_t.core.entities.entity_manager import get_entity_manager, EntityType
import uuid
import faulthandler
...
# Assuming this script is in merl_t/core/annotation/, the root is 3 levels up
try: # Disabled migrations
...
def setup_environment():
    ...
    try:
        ...
        # Determina project_root senza modificare sys.path
        project_root = current_dir.parents[2]  # Assumendo che siamo in merl_t/core/annotation
        ...
        # Check if PostgreSQL configuration is available and enabled
        use_postgres = False
        
        if config_manager:
            db_config = config_manager.get_section('database')
            if db_config and db_config.get('type') == 'postgresql':
                # Check if all required PostgreSQL connection parameters are provided
                connection_params = db_config.get('connection', {})
                required_params = ['host', 'port', 'user', 'password', 'database']
                if all(connection_params.get(param) for param in required_params):
                    use_postgres = True
                    annotation_logger.info("PostgreSQL configuration detected, attempting to connect")
                else:
                    missing_params = [param for param in required_params if not connection_params.get(param)]
                    annotation_logger.warning(f"PostgreSQL configuration incomplete. Missing parameters: {missing_params}")
            else:
                annotation_logger.info("PostgreSQL not configured in settings, using mock database")
        
        if use_postgres:
            try:
                # Try to initialize PostgreSQL database manager
                db_manager = AnnotationDBManager()
                annotation_logger.info("Successfully connected to PostgreSQL database")
                
                # Esegui le migrazioni del database
                try:
                    from .db_migrations import run_migrations_for_postgres
                    annotation_logger.info("Esecuzione migrazioni database PostgreSQL...")
                    run_migrations_for_postgres()
                    annotation_logger.info("Migrazioni database completate.")
                except Exception as migration_error:
                    annotation_logger.error(f"Errore durante l'esecuzione delle migrazioni: {migration_error}")
                
                # Assicurati che esista l'utente admin
                ensure_admin_exists(db_manager)
                
            except Exception as db_error:
                annotation_logger.error(f"Failed to connect to PostgreSQL: {db_error}")
                use_postgres = False
                annotation_logger.info("Falling back to mock database implementation")
        
        # If PostgreSQL is not available or not configured, use the mock implementation
        if not use_postgres:
            annotation_logger.info("Initializing mock database manager...")
            
            # Create a mock database manager implementation
            class MockDBManager:
                def __init__(self):
                    self.logger = logging.getLogger("mock_db_manager")
                    self.logger.info("Mock DB Manager initialized")
                    
                def get_all_users(self, active_only=False):
                    return [{"id": "admin_user", "username": "admin", "role": "admin"}]
                ...
                def get_documents(self, status=None, assigned_to=None):
                    return []
                    
                def get_annotations(self, doc_id=None):
                    return {}
                ...
                def __getattr__(self, name):
                    def method(*args, **kwargs):
                        self.logger.info(f"Mock method call: {name}({args}, {kwargs})")
            ...
            annotation_logger.info("Mock database manager initialized successfully")
        ...
        annotation_logger.error(f"Error during environment setup: {e}", exc_info=True)
        raise
...
try: # Disabled migrations
    ...
    # Utilizzo importazioni assolute
    try: # Disabled migrations
        from merl_t.core.entities.entity_manager import get_entity_manager
        from merl_t.core.ner import NERSystem
        annotation_logger.info("Moduli importati con successo tramite percorso assoluto")
    except ImportError as e:
        annotation_logger.warning(f"Impossibile importare tramite percorso assoluto: {e}")
        # Fallback con implementazioni fittizie
...
# Configura i percorsi dei dati in base alla nuova struttura
DATA_DIR = project_root / "merl_t" / "data" / "annotation"
...
def cleanup_backups():
    ...
    Utilizza la configurazione per determinare il numero massimo di backup.
    """
    try: # Disabled migrations
        ...
        # Passa il valore al metodo del db_manager
...
def train_model():
    ...
    try: # Disabled migrations
        ...
        output_dir = data.get('output_dir', config_manager.get('training.standard.output_dir', 'merl_t/core/data/models/transformer'))
        ...
        training_data_dir = config_manager.get('training.standard.train_data_dir', 'merl_t/core/data/training_data')
        ...
        output_file = config_manager.get('training.standard.output_file', 'merl_t/core/data/training_data.json')
...
def train_with_feedback():
    ...
    try: # Disabled migrations
        ...
        feedback_dir = config_manager.get('annotation.feedback.storage_dir', 'merl_t/core/annotation/data/feedback')
        ...
        output_dir = config_manager.get('training.reinforcement.output_dir', 'merl_t/core/data/models/transformer_feedback')
        if not os.path.isabs(output_dir):
            output_dir = os.path.join(ROOT_DIR, output_dir)
...
def api_run_knowledge_graph_extraction():
    ...
    try: # Disabled migrations
        ...
        module_path = os.path.abspath('merl_t/core/kg/graph_main.py')
...
def apply_graph_proposal(proposal_id):
    ...
    try: # Disabled migrations
        ...
        # Importa la funzione apply_kg_changes dal nuovo percorso
        from merl_t.core.kg import apply_kg_changes
    ...
    try: # Disabled migrations
        ...
        async def background_task():
            try: # Disabled migrations
                ...
                # Importa create_node_centric_chunks dal nuovo percorso
                from merl_t.core.kg import create_node_centric_chunks
