...
def process_directory(input_dir, output_dir, file_type='json', num_workers=0, dry_run=False, verbose=False):
    """
    Processa tutti i file in una directory.
    
    Args:
        input_dir: Directory contenente i file
        output_dir: Directory dove salvare i file puliti
        file_type: Tipo di file da elaborare ('json' o 'txt')
    ...
    logger.info(f"Elaborazione di {len(files)} file {file_type.upper()} in {input_dir}")
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        futures = [executor.submit(processing_func, file_path, output_dir, cleaner, dry_run) 
                 for file_path in files]
        
        # Processa i risultati man mano che sono disponibili
        for future in tqdm(as_completed(futures), total=len(futures), desc=f"Pulizia {file_type}"):
    ...
    stats_file = os.path.join(output_dir if not dry_run else '.', f"cleaning_stats_{file_type}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Script per la pulizia in batch di file')
    
    parser.add_argument('--input', '-i', required=True, 
                       help='Directory contenente i file da pulire')
    parser.add_argument('--output', '-o', 
                       help='Directory dove salvare i file puliti')
    parser.add_argument('--type', '-t', choices=['json', 'txt'], default='json',
                       help='Tipo di file da elaborare (json o txt)')
    ...
    if not args.dry_run and not args.output:
        print(f"Errore: la directory di output è obbligatoria quando non in modalità dry-run")
        return 1
