...
# -----------------------------------------------------------------------------
# IMPOSTAZIONI GENERALI DATABASE
# -----------------------------------------------------------------------------

# Otteniamo il percorso del database dalla configurazione o dal percorso predefinito
def get_db_path():
    """
    Ottiene il percorso del database dalle configurazioni o dal percorso predefinito.
    ...
    if config_manager:
        db_path = config_manager.get("annotation.database.path")
        if db_path:
            return db_path
    
    # Percorso predefinito: merl_t/data/annotation/annotations.db
    project_root = Path(__file__).resolve().parents[3]
    return str(project_root / "merl_t" / "data" / "annotation" / "annotations.db")

def get_backup_dir():
    """
    Ottiene la directory per i backup dalle configurazioni o dal percorso predefinito.
    ...
    config_manager = get_config_manager()
    if config_manager:
        backup_dir = config_manager.get("annotation.database.backup_dir")
        if backup_dir:
            return backup_dir
    
    # Percorso predefinito: merl_t/data/annotation/backup
    project_root = Path(__file__).resolve().parents[3]
...
class AnnotationDBManager:
    ...
    def __init__(self):
        ...
        self.db_params = {
            'host': db_config.get('connection', {}).get('host', 'localhost'),
            'port': db_config.get('connection', {}).get('port', 5432),
            'user': db_config.get('connection', {}).get('user'),
            'password': db_config.get('connection', {}).get('password'),
            'dbname': db_config.get('connection', {}).get('database')
        }

        if not all(self.db_params.values()):
            raise ValueError("Parametri di connessione PostgreSQL mancanti in config.yaml (connection.host, connection.port, connection.user, connection.password, connection.database)")
