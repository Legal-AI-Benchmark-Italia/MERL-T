...
class EntityNormalizer:
    ...
    def __init__(self, entity_manager=None):
        ...
        self.enable = config_manager.get("normalization.enable", True)
        ...
        self.use_knowledge_graph = config_manager.get("normalization.use_knowledge_graph", False)
    ...
    def _load_canonical_forms(self) -> Dict[str, Dict[str, Any]]:
        ...
        canonical_forms_file = config_manager.get("normalization.canonical_forms_file", "data/canonical_forms.json")
        
        canonical_forms = {}
        
        try:
            # Verifica che il percorso sia assoluto o relativo
            file_path = Path(canonical_forms_file)
            if not file_path.is_absolute():
                # Se è relativo, usalo rispetto alla directory del progetto
                file_path = Path(ROOT_DIR) / canonical_forms_file
    ...
    def _load_abbreviations(self) -> Dict[str, str]:
        ...
        abbreviations_file = config_manager.get("normalization.abbreviations_file", "data/abbreviations.json")
        
        abbreviations = {}
        
        try:
            # Verifica che il percorso sia assoluto o relativo
            file_path = Path(abbreviations_file)
            if not file_path.is_absolute():
                # Se è relativo, usalo rispetto alla directory del progetto
                file_path = Path(ROOT_DIR) / abbreviations_file
    ...
    def _setup_knowledge_graph(self):
        ...
        try:
            ...
            kg_config = config_manager.get_neo4j_connection_params()
            
            url = kg_config.get("uri", "bolt://localhost:7687")
