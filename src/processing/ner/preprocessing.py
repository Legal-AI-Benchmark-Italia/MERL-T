...
class TextPreprocessor:
    ...
    def __init__(self):
        ...
        self.use_spacy = config_manager.get("preprocessing.tokenization.use_spacy", True)
        self.normalize_spaces = config_manager.get("preprocessing.normalization.normalize_spaces", True)
        self.lowercase = config_manager.get("preprocessing.normalization.lowercase", False)
        self.remove_accents = config_manager.get("preprocessing.normalization.remove_accents", False)
        
        # Carica il modello spaCy se necessario
        if self.use_spacy:
            spacy_model = config_manager.get("models.spacy.model_name", "it_core_news_lg")
            disabled_components = config_manager.get("models.spacy.disable", ["ner"])
    ...
    def segment_text(self, text: str) -> List[str]:
        ...
        use_spacy_segmentation = config_manager.get("preprocessing.segmentation.use_spacy", True)
        max_segment_length = config_manager.get("preprocessing.segmentation.max_segment_length", 512)
        overlap = config_manager.get("preprocessing.segmentation.overlap", 128)
