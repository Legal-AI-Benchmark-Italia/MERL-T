...
def find_project_root():
    ...
    # Se siamo già nella directory ner. il parent è la root
    if current_dir.name == 'ner.:
        return current_dir.parent
    
    # Altrimenti, cerchiamo la directory che contiene sia 'ner. che 'config'
    while current_dir != current_dir.parent:
        if (current_dir / 'ner.).exists() and (current_dir / 'config').exists():
            return current_dir
...
def test_entity_manager_integration():
    ...
    """
    try:
        from ner.entities.entity_manager import get_entity_manager
...
def test_converter_module():
    ...
    try:
        ...
        from ner.utils.converter import (
...
def test_ner_training_module():
    ...
    try:
        ...
        sys.path.append(str(find_project_root()))
        from ner.training.ner_trainer import NERTrainer, train_from_annotations
        ...
        try:
            ...
            # Converti in formato spaCy
            from ner.utils.converter import convert_annotations_to_spacy_format
            spacy_data = convert_annotations_to_spacy_format(annotations, documents)
